...
fn test_slide1() {
    ...
    assert!(positions
        .iter()
        .any(|p| p.position.is_occupied_by_piece(G5, WHITE_QUEEN)
            && p.position.is_occupied_by_piece(G7, WHITE_PAWN)));
    assert!(positions
        .iter()
        .any(|p| p.position.is_occupied_by_piece(G6, WHITE_QUEEN)
            && p.position.is_occupied_by_piece(G7, WHITE_PAWN)));
}
...
fn test_get_piece_moves_king() {
    ...
    assert!(contains_move(&chess_move, WHITE_KING, D5)); // up
    assert!(contains_move(&chess_move, WHITE_KING, D3)); // down
    assert!(contains_move(&chess_move, WHITE_KING, C4)); // left
    assert!(contains_move(&chess_move, WHITE_KING, E4)); // right
}
...
fn test_slide2() {
    ...
    assert!(positions
        .iter()
        .any(|p| p.position.is_occupied_by_piece(G5, WHITE_QUEEN)
            && p.position.is_occupied_by_piece(G7, BLACK_PAWN)));
    assert!(positions
        .iter()
        .any(|p| p.position.is_occupied_by_piece(G6, WHITE_QUEEN)
            && p.position.is_occupied_by_piece(G7, BLACK_PAWN)));
    assert!(positions
        .iter()
        .any(|p| p.position.is_occupied_by_piece(G7, WHITE_QUEEN)
            && !p.position.is_occupied_by_piece(G7, BLACK_PAWN)));
}
...
fn test_slide3() {
    ...
    assert!(positions
        .iter()
        .any(|p| p.position.is_occupied_by_piece(G8, WHITE_QUEEN)));
}
