...
pub fn count(position: &Position, color: Color) -> f32 {
    let mut score: f32 = 0.0;
    score += get_doubled_pawns(position, color);
    score += get_isolated_pawns(position, color);
    score += get_passed_pawns(position, color);
    score
}
pub fn get_doubled_pawns(position: &Position, color: Color) -> f32 {
    let mut score = 0.0;
    for mut square in position.get_squares(color, Typ::Pawn).iter() {
        let mut has_pawn_in_front = false;

        while let Some(front_square) = directions::up(square) {
            if position.is_occupied_by(front_square, color, Typ::Pawn) {
...
pub fn get_isolated_pawns(position: &Position, color: Color) -> f32 {
    let mut score: f32 = 0.0;
...
pub fn get_passed_pawns(position: &Position, color: Color) -> f32 {
    let mut score = 0.0;
    let opponent_pawn = Piece::new(color.get_opponent_color(), Typ::Pawn);
    let direction_fn = match color {
        Color::Black => directions::down,
        Color::White => directions::up,
    };
